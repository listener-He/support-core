...
public class LinearComputeUtil {
    ...
    /**
     * 根据给定的列表和两个函数，生成一个映射，该映射的键是一个谓词，用于判断列表中的元素是否在某个区间内，
     * 值是列表中的元素本身。
     *
     * @param list  包含T类型元素的列表。
     * @param left  一个函数，用于确定区间的左端点。
     * @param right 一个函数，用于确定区间的右端点。
     *
     * @return {@link Map }<{@link Predicate }<{@link E }>, {@link T }> 返回一个映射，键是谓词，用于判断列表中的元素是否在指定的区间内，值是列表中的元素。
     */
    public static <E extends Comparable<E>, T> Map<Predicate<E>, T> section(List<T> list, Function<? super T, ? extends E> left, Function<? super T, ? extends E> right) {
    ...
    /**
     * 连续区间分段线性函数，传入区间终点和区间内权重，返回此分段线性函数(区间连续，所以区间结尾是否取值不影响结果)，权重为空返回原值,超出最后一个区间取值为最后一个区间权重
     * 求一个 跳跃（可去）间断点函数的积分函数是连续（可导）的
     * f(x)= a (x < interval1) 或 f(x)= a (x <= interval1)
     * f(x)= b (x < interval2)  或 f(x)= b (x <= interval2)
     * 的不定积分
     * F（x）= a*x (x <= interval1)
     * F（x）= a*interval1 + b*(x-interval1) = b*x - (b-a)*interval1  (interval1 <=x < interval2)
     * F(interval1+0) = F(interval1-0)
     *
     * @param intervalEndWeightMap 区间终点和权重的映射，用于定义分段线性函数的各个区间和对应的斜率
     *
    ...
    public static UnaryOperator<BigDecimal> piecewiseLinear(Map<BigDecimal, BigDecimal> intervalEndWeightMap) {
        ...
        BigDecimal[] intervalEnds = weightMap.keySet().stream().sorted(BigDecimal::compareTo).toArray(BigDecimal[]::new);
        ...
        for (int i = 1; i < intervalEnds.length; i++) {
            fastDeduction[i] = weightMap.get(intervalEnds[i]).multiply(intervalEnds[i - 1])
                .subtract(weightMap.get(intervalEnds[i - 1]).multiply(intervalEnds[i - 1]).subtract(fastDeduction[i - 1]));
        }
    ...
    /**
     * 连续区间分段线性函数，传入区间终点和区间内权重，返回此分段线性函数(区间连续，所以区间结尾是否取值不影响结果)，权重为空返回原值,负数结果全为0
     * 求一个 跳跃（可去）间断点函数的积分函数是连续（可导）的
     * f(x)= a (x < interval1) 或 f(x)= a (x <= interval1)
     * f(x)= b (x < interval2)  或 f(x)= b (x <= interval2)
     * 的不定积分
     ...
     * F(interval1+0) = F(interval1-0)
     *
     * @param intervalEndWeightMap 区间和权重映射，仅传入区间终点（默认不包含），区间必须连续
     *
     * @return {@link BiFunction }<{@link BigDecimal }, {@link BigDecimal }, {@link BigDecimal }> 分段函数(区间计算)
     */
    public static BiFunction<BigDecimal, BigDecimal, BigDecimal> piecewiseIntervalPositive(Map<BigDecimal, BigDecimal> intervalEndWeightMap) {

        Map<BigDecimal, BigDecimal> positiveInterval = Optional.ofNullable(intervalEndWeightMap).orElse(Collections.emptyMap()).entrySet().stream().filter(e -> e.getKey().compareTo(ZERO) > 0).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
     ...
     * 线性区间
     *
     * @param intervalEndWeightMap 区间末端权重
     *
     * @return {@link BiFunction }<{@link BigDecimal }, {@link BigDecimal }, {@link BigDecimal }>
     */
    public static BiFunction<BigDecimal, BigDecimal, BigDecimal> linearInterval(Map<BigDecimal, BigDecimal> intervalEndWeightMap) {
