...
public class CollUtil {
    ...
    public static <T, K> List<T> distinct(Collection<T> collection, Function<T, K> keyFunction) {
        ...
        return new ArrayList<>(collection.stream().filter(e -> keyFunction.apply(e) != null).collect(Collectors.toMap(keyFunction, Function.identity(), BeanUtil::mergeFirst))
            .values());
    }
     ...
     */
    @SafeVarargs
    public static <T> List<T> sort(Collection<T> collection, boolean reverse, Function<T, ? extends Comparable<Object>>... sortFields) {
        if (EmptyUtil.isEmpty(collection)) {
            return Collections.emptyList();
        }
     ...
     *
     * @param collection  集合
     * @param getValue    获取集合属性值
     * @param accumulator 属性值计算
     * @param other       其他
     *
     * @return {@link R }
     */
    public static <T, R> R reduce(Collection<T> collection, Function<T, R> getValue, BinaryOperator<R> accumulator, Supplier<? extends R> other) {
        if (EmptyUtil.isEmpty(collection)) {
            return other.get();
        }
     ...
     *
     * @param collection     需要被处理的集合，不能为null且不可为空。
     * @param stringFunction 一个函数，用于将集合中的元素转换为字符串。
     * @param separator      用于连接转换后的字符串的分隔符。
     *
    ...
    /**
     * 将列表元素转换为映射，其中键是元素在列表中的索引
     * 此方法的目的是为了快速通过索引访问列表元素，提高查询效率
     *
     * @param list 列表，其中包含要转换为映射的元素
     *
    ...
    /**
     * 将集合转换为映射，其中映射的键和值由提供的函数生成
     * 此方法允许从一个集合创建一个映射，通过指定的函数生成键和值
     * 如果集合中的元素生成了相同的键，将使用指定的合并函数进行合并
     *
     * @param collection    要转换的集合
     * @param keyFunction   用于生成映射键的函数
     * @param valueFunction 用于生成映射值的函数
     *
    ...
    /**
     * 将给定的集合转换成一个Map，通过提供的keyMapper和valueMapper函数来提取键和值，当遇到重复的键时，使用mergeFunction来合并值。
     * 该方法使用提供的Map供应商来创建新的Map实例。
     *
     * @param items         集合对象，将被转换为Map。如果为null或空集合，将返回一个空的Map。
     * @param keyMapper     一个函数，用于根据输入项生成Map的键。
     * @param valueMapper   一个函数，用于根据输入项生成Map的值。
     * @param mergeFunction 当遇到重复键时应用的合并函数，用于合并值。
     * @param supplier      一个供应函数，用于提供一个新的空Map实例。
     * @param <T>           输入项的类型。
     * @param <K>           Map键的类型。
     * @param <V>           Map值的类型。
     *
     ...
     * @return 平均数
     */
    public static <T, E extends Number> double average(List<T> list, Function<T, E> getElement, BinaryOperator<E> accumulator, Function<E, Double> toDouble) {
        ...
        if (list instanceof RandomAccess) {
            ...
            // 对于顺序存取的，采用迭代访问
            list.stream().map(getElement).reduce(accumulator).orElseThrow(() -> new IllegalArgumentException("list is empty"));
        }
    ...
    /**
     * 根据给定的节点列表和起始节点ID构建树形结构
     * 此方法用于从一个扁平的节点列表中构建出树形结构，便于层次展示或处理
     *
     * @param list    节点列表，每个节点都是一个TreeNode的实现
     * @param startId 起始节点的ID，用于定位树的根节点
     *
     * @return 返回构建好的树形结构列表
     */
     ...
     * 将列表转换为树结构
     *
     * @param list         列表，待转换为树结构的源数据集合。
     * @param parentGetter 父ID的获取函数，用于从列表元素中获取父节点的ID。
     * @param idGetter     当前节点ID的获取函数，用于从列表元素中获取当前节点的ID。
     * @param childGetter  子节点列表的获取函数，用于从列表元素中获取子节点的集合。
     * @param startId      开始节点ID，指定树结构的根节点ID。
     * @param <R>          节点类型，列表元素的类型。
     * @param <I>          ID类型，节点ID的类型。
     *
     * @return 转换后的树结构列表，返回根节点的集合。
     */
    public static <R, I> List<R> tree(List<R> list, Function<? super R, ? extends I> parentGetter,
                                      Function<? super R, ? extends I> idGetter,
        ...
        List<R> trees = list.stream().filter(node -> Objects.equals(parentGetter.apply(node), startId)).collect(Collectors.toList());
     ...
     *
     * @param map         映射表，用于存储节点标识符到节点对象的映射。
     * @param currentNode 当前处理的节点对象，将以此节点作为树结构的构建起点。
     * @param idGetter    一个函数，用于从节点对象中获取节点的标识符。
     * @param childGetter 一个函数，用于从节点对象中获取该节点的子节点列表。
     *                    <p>
     *                    通过递归方式遍历映射表，根据当前节点及其getter函数，构建从当前节点开始的树结构。
     *                    首先，获取当前节点的标识符和其对应的子节点列表。然后，将找到的子节点添加到当前节点的子节点列表中。
     *                    最后，对每个子节点递归调用此方法，以构建整棵树。
     */
    private static <R, I> void buildTree(Map<I, List<R>> map, R currentNode,
                                         Function<? super R, ? extends I> idGetter, Function<? super R, List<R>> childGetter) {
        // 获取当前节点的标识符
    ...
    /**
     * 比较繁琐 复杂的比较器?? 但是相对安全
     * 创建一个比较器，该比较器基于提供的函数对对象进行比较
     * 这些函数提取出对象中用于比较的字段，字段必须是可比较的
     *
     * @param <T>    要比较的对象类型
     * @param fields 一个或多个函数，用于提取出对象中用于比较的字段
     *
