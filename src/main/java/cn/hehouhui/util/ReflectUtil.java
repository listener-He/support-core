...
public class ReflectUtil {
    ...
    public static <T> Constructor<T> getConstructor(Class<T> type, Class<?>... parameterTypes) {
        try {
            ...
            throw new CommonException(ErrorCodeEnum.CODE_ERROR, StrUtil.format("类[{}]中不存在参数列表为[{}]的构造器", type,
                parameterTypes == null ? BasicConstant.EMPTY : Arrays.toString(parameterTypes)), e);
        }
    }
    ...
    public static <R> R invoke(Object obj, String methodName, Class<?>[] parameterTypes, Object... args) {
        Assert.assertTrue(CollUtil.sizeEquals(parameterTypes, args), "方法参数类型列表必须和方法参数列表一致",
    ...
    public static <R> R invoke(Object obj, Method method, Object... args) {
        try {
            ...
            throw new CommonException(ErrorCodeEnum.REFLECT_SECURE_EXCEPTION,
                StrUtil.format("当前没有权限调用方法或者方法参数错误，方法：[{}],参数:[{}]", method, args), e);
        } catch (InvocationTargetException e) {
            Throwable realEx = e.getTargetException();
            ErrorCodeEnum code = ErrorCodeEnum.UNKNOWN_EXCEPTION;
            if (realEx instanceof BaseEnum) {
                code = ((CommonException)realEx).getErrCode();
            }
            throw new CommonException(code, StrUtil.format("方法执行异常，调用的方法：[{}].参数：[{}]", method, args),
    ...
    public static Method getMethod(Class<?> clazz, String methodName, boolean recursive, Class<?>... parameterTypes) {
        ...
        if (method == null) {
            throw new CommonException(ErrorCodeEnum.CODE_ERROR, StrUtil.format("类[{}]中不存在方法名为[{}]、方法列表为[{}]的方法", clazz,
                methodName, parameterTypes == null ? BasicConstant.EMPTY : Arrays.toString(parameterTypes)));
        }
        return method;
    }
    ...
    public static <T> T getFieldValue(Object obj, Field field) {
        ...
        try {
            ...
            String msg = StrUtil.format("类型[{}]的字段[{}]不允许访问", obj.getClass(), field.getName());
    ...
    public static <T> void setFieldValue(Object obj, Field field, T fieldValue) {
        ...
        try {
            ...
            String msg = StrUtil.format("类型[{}]的字段[{}]不允许设置", obj.getClass(), field.getName());
            throw new CommonException(ErrorCodeEnum.REFLECT_SECURE_EXCEPTION, msg, e);
        }
    }
    ...
    public static Field getField(Object obj, String fieldName, boolean isRecursive, boolean throwIfAbsent) {
        ...
        try {
            ...
            if (superClass.equals(Object.class) || !isRecursive) {
                if (throwIfAbsent) {
                    throw new CommonException(ErrorCodeEnum.REFLECT_SECURE_EXCEPTION,
                        StrUtil.format("[{}]中不存在字段[{}]", obj, fieldName));
                } else {
                    return null;
                }
            } else {
    ...
    public static <T extends AccessibleObject> T allowAccess(T object) {
        try {
            ...
            throw new CommonException(ErrorCodeEnum.REFLECT_SECURE_EXCEPTION, StrUtil.format("无法更改[{}]的访问权限", object),
                e);
        }
        return object;
    }
    ...
    private static Method getMethodIfExist(Class<?> clazz, String methodName, boolean recursive,
        ...
        try {
            ...
            // 如果不允许递归，直接抛出异常
            if (!recursive) {
                throw new CommonException(ErrorCodeEnum.CODE_ERROR, StrUtil.format("类[{}]中不存在方法名为[{}]、方法列表为[{}]的方法",
                    clazz, methodName, parameterTypes == null ? BasicConstant.EMPTY : Arrays.toString(parameterTypes)),
                    e);
            }
        }
