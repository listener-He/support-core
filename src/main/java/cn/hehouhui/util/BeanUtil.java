...
public class BeanUtil {
    ...
    /**
     * 创建并返回一个 {@link BinaryOperator}，它将两个输入元素中的第一个作为结果返回。
     * 这个操作符主要用于合并或联合两个同类型元素时，选择第一个元素作为结果。
     *
     * @param <U> 指定二元操作符操作的元素类型。
     *
    ...
    public static <I, N, E, R> R mapThenJoin(Collection<? extends I> collection,
                                             ...
                                             BiFunction<? super I, ? super N, ? extends E> mapper,
                                             Collector<? super E, ?, R> collector) {
        ...
        return filtered.stream().map(id -> mapper.apply(id, nameMap.get(id))).filter(Objects::nonNull).collect(collector);
    }
    ...
    /**
     * 为集合中的每个对象补全名称信息
     * 该方法通过提供的ID到名称的映射函数和设置/获取函数对，为集合中每个对象的名称字段进行补全
     *
     * @param <I>             对象ID的类型
     * @param <N>             名称的类型
     * @param <E>             集合元素的类型
     * @param collection      需要处理的集合
     * @param nameMapCreator  根据ID列表创建ID到名称的映射的函数
     * @param setGetFunctions 一组包含ID获取器和名称设置器的函数对
     */
    @SafeVarargs
    public static <I, N, E> void complete(Collection<? extends E> collection, Function<? super List<I>, ? extends Map<? super I, ? extends N>> nameMapCreator, SetGet<E, I, N>... setGetFunctions) {
        ...
        List<I> idSet = collection.stream().flatMap(item -> collect.stream().map(f -> f.getIdGetter().apply(item))).filter(Objects::nonNull).distinct().collect(Collectors.toList());
        ...
        collection.stream().filter(Objects::nonNull).forEach(e -> {
            collect.forEach(func -> {
                Function<? super E, ? extends I> idGetter = func.getIdGetter();
                BiConsumer<? super E, ? super N> nameSetter = func.getNameSetter();
                // 提取对象的ID
    ...
    /**
     * 补全 collection 中的名称
     * 通过从集合中提取的ID集合查询出对应的name
     *
     * @param collection     数据源
     * @param idGetter       id 提取
     * @param nameSetter     名称设置
     * @param nameMapCreator id与名称 Map数据函数
     */
    public static <I, N, E> void complete(Collection<? extends E> collection,
        ...
        // 提取id集合
        List<I> idSet = collection.stream().filter(Objects::nonNull)
            .map(idGetter)
     ...
     *
     * @param collection     数据源集合，需要补全名称的对象集合。
     * @param idGetter       从集合对象中提取ID的函数接口。
     * @param nameSetter     设置集合对象名称的双参数消费者接口。
     * @param nameMapCreator 提供ID-名称映射的供应者接口。
     * @param <I>            ID的类型。
     * @param <N>            名称的类型。
     * @param <E>            集合对象的类型。
     */
    public static <I, N, E> void complete(Collection<? extends E> collection,
                                          Function<? super E, ? extends I> idGetter,
                                          BiConsumer<? super E, ? super N> nameSetter,
                                          Supplier<? extends Map<? super I, ? extends N>> nameMapCreator) {
        // 如果集合为空，则直接返回，无需任何操作
        if (EmptyUtil.isEmpty(collection)) {
            return;
        }
     ...
     * @date 2024/11/19
     */
    public static class SetGet<E, I, N> {
        ...
        public Function<? super E, ? extends I> getIdGetter() {
            return idGetter;
        }

        public BiConsumer<? super E, ? super N> getNameSetter() {
            return nameSetter;
        }
    }
