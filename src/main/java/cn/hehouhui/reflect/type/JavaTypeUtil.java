...
public class JavaTypeUtil {
    ...
    public static JavaType createJavaType(Type type, LinkedHashMap<String, JavaType> bindings) {
        ...
        JavaType javaType;
        if (type instanceof final WildcardType wildcardTypeImpl) {
        ...
        } else if (type instanceof final TypeVariable<?> typeVariableImpl) {
            ...
            genericType.setRawType(rawType);
            javaType = genericType;
        } else if (type instanceof final ParameterizedType parameterizedTypeImpl) {
            // 该类型存在泛型
     ...
     */
    public static CustomGenericArrayType getArrayDesc(Type type) {
     ...
     * @return 数组说明，如果指定类型不是数组，那么抛出异常
     */
    public static CustomGenericArrayType getArrayDesc(Type type, LinkedHashMap<String, JavaType> resolved) {
     ...
     * @return 数组说明
     */
    private static CustomGenericArrayType getArrayDesc(Type type, int now, LinkedHashMap<String, JavaType> resolved) {
    ...
    private static LinkedHashMap<String, JavaType> getBindings(JavaType javaType) {
        if (javaType instanceof SimpleType) {
            ...
            throw new UnsupportedOperationException(
                StrUtil.format("不支持的操作类型: [{}]", javaType == null ? null : javaType.getClass()));
        }
    }
    ...
    public static List<JavaType> getDeclareGenerics(Class<?> clazz) {
        TypeVariable<? extends Class<?>>[] typeParameters = clazz.getTypeParameters();
        if (EmptyUtil.isEmpty(typeParameters)) {
            return Collections.emptyList();
        }
    ...
    public static Class<?> signToClass(String jvmClassName) {
        return signToClass(jvmClassName, ClassUtil.getDefaultClassLoader());
    }
    ...
    public static Class<?> signToClass(String jvmClassName, ClassLoader classLoader) {
        return ClassUtil.loadClass(signToClassName(jvmClassName), classLoader);
    }
    ...
    public static String signToClassName(String jvmClassName) {
        ...
        if (baseClass == null) {
            throw new CommonException(ErrorCodeEnum.UNKNOWN_EXCEPTION, StrUtil.format("指定类型[{}]无法解析", packageClass));
        }
