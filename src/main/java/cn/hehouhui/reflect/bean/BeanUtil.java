...
public class BeanUtil {
    ...
    /**
     * 获取对象中指定字段的值
     * 此方法用于批量获取对象中指定字段的值如果目标对象为null或字段名称列表为空，
     * 则返回一个空的映射通过将目标对象转换为映射，并根据字段名称列表进行过滤和收集，
     * 最终返回一个包含指定字段及其对应值的新映射如果指定的字段在目标对象中不存在，
     * 则该字段在返回的映射中的值为空字符串
     *
     * @param target     目标对象，从该对象中提取字段值
     * @param fieldNames 字段名称列表，表示需要提取的字段
     *
    ...
    /**
     * 比较两个对象的差异，忽略指定的属性
     * 此方法用于识别两个对象之间属性值的差异，主要应用于对象比较或同步场景
     * 它通过将对象转换为Map来比较它们的属性值，并返回不同值的属性名和对应的新值
     *
     * @param target   目标对象，代表被比较的新对象
     * @param original 原始对象，代表被比较的旧对象
     * @param ignore   忽略比较的属性名列表，这些属性将不参与比较
     *
